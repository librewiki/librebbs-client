...
@Component({
  ...
  metaInfo(): MetaInfo {
    return {
      title: (this as SearchPage).board.display_name,
    };
  },
})
export default class SearchPage extends Vue {
  ...
  async handleInfinite($state: StateChanger): Promise<void> {
    const searchType = 1;
    const limit = 10;
    const topics = await searchComment(
      this.board.id,
      searchType,
  ...
  mounted(): void {
    if (this.$route.params.query.length < 2) {
      store.commit(
        "setErrormodal",
        "검색어는 최소 2글자부터 가능합니다."
      );
      this.$router.push(
        "/" + String(this.board.name)
      );
    }
    if (
